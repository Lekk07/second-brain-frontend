    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-ink-muted text-sm mt-1">Start building your knowledge vault</p>
        </div>

        <div className="glass rounded-2xl p-8 border border-border shadow-card">
          <h2 className="font-display font-semibold text-lg text-ink-primary mb-6">Create account</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-mono text-ink-muted mb-1.5">Username</label>
              <input value={username} onChange={e => setUsername(e.target.value)}
                placeholder="yourname" required className="input-field" />
            </div>
            <div>
          ...
          <p className="text-center text-sm text-ink-muted mt-6">
            ...
            <Link to="/login" className="text-accent hover:text-accent-glow transition-colors font-medium">Sign in</Link>
          </p>
        </div>
      </motion.div>
    </div>
